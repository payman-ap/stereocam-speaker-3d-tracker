...
source_path = './callibration_data' 
#print('images found :',len(os.listdir(source_path))) # count number of images

# Create list with directories of each image    
images = glob.glob('./callibration_data/*.jpeg')
#print(images)
found = 0
pat_directory = './Patterned_images'
...
for fname in images: # here, 10 can be changed to whatever number you like to choose
    ...
    if ret == True:
        ...
        cv2.waitKey(50)
        #cv2.imwrite(pat_directory + '/pat_img' + str(found) + '.jpeg', jpeg)
...
# undistort image
undis_directory = './Undistorted_images'
undis_cropped_directory = './Undistorted_cropped_images'
found = 0
for fname in images: # here, 10 can be changed to whatever number you like to choose
     #print(fname)
     jpeg = cv2.imread(fname) # Capture frame-by-frame
     ...
     cv2.waitKey(500)
     #cv2.imwrite(undis_directory + '/imdist_img' + str(found) + '.jpeg', dst)
     ...
     cv2.waitKey(500)
     #cv2.imwrite(undis_cropped_directory + '/imdist_cropped_img' + str(found) + '.jpeg', dst)
